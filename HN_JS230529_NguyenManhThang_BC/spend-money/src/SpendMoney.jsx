...
function SpendMoney() {
  ...
  const handlePlusItem = (item) => {
    const exitsItem = cartItems.find((cartItem) => cartItem.id === item.id);

    if (exitsItem) {
      exitsItem.quantity += 1;

      const cartUpdateQuantity = cartItems.map((cartItem) =>
        cartItem.id === exitsItem.id ? exitsItem : cartItem
      );
      setCartItems(cartUpdateQuantity);
    } else {
  ...
  const handleMinusItem = (product) => {
    const exitsItem = cartItems.find((item) => item.id === product.id);

    if (exitsItem && exitsItem.quantity > 1) {
      exitsItem.quantity -= 1;
    }

    const cartUpdateQuantity = cartItems.map((item) =>
      item.id === exitsItem.id ? exitsItem : item
    );
  ...
  const handleDelete = (item) => {
    const newCartItem = cartItems.filter((cart) => cart.id !== item.id);
